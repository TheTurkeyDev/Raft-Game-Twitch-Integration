...
public static class AI_NetworkBehavior_Shark_Serialize_Update_Animal_Patch
{
    static void Postfix(AI_NetworkBehavior_Shark __instance)
    {
        if (Raft_Network.IsHost)
        {
            ...
            RAPI.SendNetworkMessage(new UpdateSharkNameMessage(TwitchItegration.MESSAGE_TYPE_SET_NAME, __instance.ObjectIndex, name), channel: TwitchItegration.CHANNEL_ID);
        }
    }
}